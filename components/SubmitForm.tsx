...
async function submitListing(data: FormData): Promise<{
    error?: string;
}> {
...
export default function SubmitForm() {
    ...
    return (<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      {serverError && (<div className="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-xl">
          {serverError}
        </div>)}
      ...
      <div>
        ...
        <textarea {...register('bio')} rows={5} className={inputClass} placeholder="Tell patients about your approach to menopause care, your experience, and what makes your practice different..."/>
      ...
      <div className="bg-brand-cream rounded-2xl p-5 space-y-3">
        ...
        ].map(({ name, label }) => (<label key={name} className="flex items-start gap-3 cursor-pointer">
            <input type="checkbox" {...register(name)} className="mt-0.5 w-4 h-4 rounded border-gray-300 text-brand-plum focus:ring-brand-plum/30 cursor-pointer"/>
            <span className="text-sm text-gray-700">{label}</span>
          </label>))}
      </div>

      <button type="submit" disabled={isSubmitting} className="w-full bg-brand-plum hover:bg-brand-plum-dark text-white font-semibold py-4 rounded-xl transition-colors disabled:opacity-60 text-base">
