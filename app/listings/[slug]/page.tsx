...
export default async function ListingDetailPage({ params, searchParams }: PageProps) {
    ...
    return (<>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {isUnclaimed && (<div className="mx-auto mb-6 max-w-2xl rounded-lg border border-yellow-400 bg-yellow-50 px-4 py-3 text-center">
            ...
            <a href={`/claim/${listing.id}`} className="inline-block rounded-md bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">
              Claim your profile →
            </a>
          </div>)}

        {upgraded && (<div className="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-xl text-sm mb-6"> Listings are free. Public contact details are available without payment. </div>)}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
              <div className="flex items-start gap-5">
                {listing.headshot_url ? (<Image src={listing.headshot_url} alt={listing.full_name} width={96} height={96} className="w-24 h-24 rounded-full object-cover border-2 border-brand-cream-dark flex-shrink-0"/>) : (<div className="w-24 h-24 rounded-full bg-brand-cream-dark flex items-center justify-center text-brand-plum font-serif font-bold text-3xl flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h1 className="font-serif text-2xl sm:text-3xl font-bold text-gray-900 leading-tight">
                        {listing.full_name}
                        {listing.credentials && !listing.full_name.includes(listing.credentials) && (<span className="text-gray-400 font-sans font-normal text-xl">, {listing.credentials}</span>)}
                      </h1>
                      {listing.practice_name && listing.practice_name !== listing.full_name && (<p className="text-gray-500 mt-1">{listing.practice_name}</p>)}
                    </div>
                    {null}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    ...
                    {listing.mscp_certified && (<span className="text-sm font-semibold px-3 py-1 rounded-full bg-brand-plum/10 text-brand-plum border border-brand-plum/20">
                        MSCP Certified
                      </span>)}
                    {listing.hrt_prescriber && (<span className="text-sm font-medium px-3 py-1 rounded-full bg-brand-sage/10 text-brand-sage-dark">
                        HRT Prescriber
                      </span>)}
                    {listing.accepts_telehealth && (<span className="text-sm font-medium px-3 py-1 rounded-full bg-blue-50 text-blue-700 flex items-center gap-1">
                        <Wifi size={12}/>
                        Telehealth
                      </span>)}
                  </div>
              ...
              {listing.bio && (<div className="mt-6">
              ...
              {listing.specialties && listing.specialties.length > 0 && (<div className="mt-6">
                  <h2 className="font-serif text-lg font-semibold text-gray-900 mb-3">Specialties</h2>
                  <div className="flex flex-wrap gap-2">
                    {listing.specialties.map((s) => (<span key={s} className="text-sm px-3 py-1.5 rounded-lg bg-brand-cream-dark text-gray-700">
                        {SPECIALTY_LABELS[s] ?? s}
                      </span>))}
                  </div>
                </div>)}

              {listing.insurance_accepted && listing.insurance_accepted.length > 0 && (<div className="mt-6">
                  <h2 className="font-serif text-lg font-semibold text-gray-900 mb-3">Insurance Accepted</h2>
                  <div className="flex flex-wrap gap-2">
                    {listing.insurance_accepted.map((ins) => (<span key={ins} className="text-sm px-3 py-1.5 rounded-lg bg-gray-50 text-gray-600 border border-gray-100">
                        {ins}
                      </span>))}
                  </div>
                </div>)}

              {listing.languages_spoken && listing.languages_spoken.length > 1 && (<div className="mt-6">
            ...
            {isUnclaimed && (<div className="mt-6 bg-brand-cream rounded-2xl border border-brand-cream-dark p-6">
                ...
                <Link href={`/claim/${listing.id}`} className="inline-flex items-center gap-2 bg-brand-plum text-white text-sm font-medium px-5 py-2.5 rounded-full hover:bg-brand-plum-dark transition-colors">
                  Claim This Listing
                  <ArrowRight size={14}/>
                </Link>
              </div>)}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-24">
              ...
              <div className="space-y-3 text-sm mb-6">
                ...
                {listing.phone && (<div className="flex items-center gap-3 text-gray-600">
                ...
                {listing.website && (<div className="flex items-center gap-3">
                    <Globe size={16} className="flex-shrink-0 text-brand-rose"/>
                    <a href={listing.website} target="_blank" rel="noopener noreferrer" className="text-brand-plum hover:underline truncate">
              ...
              {isUnclaimed && (<div className="mb-5">
                  <PatientLeadForm listingId={String(listing.id)} providerName={listing.full_name} city={listing.city} state={listing.state}/>
                </div>)}

              {listing.booking_url ? (<a href={listing.booking_url} target="_blank" rel="noopener noreferrer" className="block w-full text-center bg-brand-plum hover:bg-brand-plum-dark text-white font-semibold py-3 px-4 rounded-xl transition-colors">
                  Book a Consultation
                </a>) : listing.phone ? (<a href={`tel:${listing.phone}`} className="block w-full text-center bg-brand-plum hover:bg-brand-plum-dark text-white font-semibold py-3 px-4 rounded-xl transition-colors">
                  Call to Schedule
                </a>) : (<Link href="/listings" className="block w-full text-center border border-brand-plum text-brand-plum font-medium py-3 px-4 rounded-xl hover:bg-brand-plum hover:text-white transition-colors">
                  Back to Search
                </Link>)}


              {isUnclaimed && (<div className="mt-4 pt-4 border-t border-gray-100 text-center">
                  <Link href={`/claim/${listing.id}`} className="text-sm text-brand-plum font-medium hover:underline">
                    Claim this listing →
                  </Link>
                </div>)}
            </div>
